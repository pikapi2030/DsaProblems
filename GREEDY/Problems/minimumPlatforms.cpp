...
the departure of another. Therefore, when two trains arrive at the same time,
or when one arrives before another departs, additional platforms are required to accommodate
both trains.
...
int findPlatform(vector<int> &arrive, vector<int> &dep)
{
    int cnt = 1;
    int ans=1;
    sort(dep.begin(), dep.end());
    sort(arrive.begin(), arrive.end());
    int i = 1, j = 0; // i at arrival, j at departure
    while (j < dep.size() && i < arrive.size())
    {
