...
snake or ladder is the start of another snake or ladder, you do not follow the subsequent
snake or ladder.
...
/*aproach
perform a bsf traversal, as we know bsf will reach in  minimum time to a cell
(inherent property of bfs)

*/

int snakesAndLadders(vector<vector<int>> &board)
{
    int n = board.size();                // also the size of column
    queue<pair<int, pair<int, int>>> qs; //{ time,{row,col} }
    qs.push({0, {0, 0}});
    while (!qs.empty())
    {
        int curTime = qs.front().first;
        int curRow = qs.front().second.first;
        int curCol = qs.front().second.second;
        qs.pop();
        vector<int> diceSimulator = {1, 2, 3, 4, 5, 6};
